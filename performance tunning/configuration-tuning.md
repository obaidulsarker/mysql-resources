...
#### Server Size: 32 Core, 128 GB Memory, SSD Disk
```
vi /etc/my.cnf
```

```
# my.cnf for 32 Cores, 128GB RAM (InnoDB Focused)
...
# InnoDB Settings (CRITICAL for performance)
innodb_buffer_pool_size        = 100G  # ~70-80% of 128GB RAM. Adjust based on other services on the server.
innodb_buffer_pool_instances   = 16    # For 100GB, 16 instances is a good start (1 per ~6-7GB)
innodb_log_file_size           = 2G    # Larger files reduce checkpoint frequency. (e.g., 256M to 4G)
innodb_log_files_in_group      = 2     # Typically 2. Total size = innodb_log_file_size * innodb_log_files_in_group
innodb_file_per_table          = 1     # Recommended: each table has its own .ibd file
innodb_flush_method            = O_DIRECT # Recommended for Linux to avoid double buffering
innodb_io_capacity             = 2000  # Adjust based on your disk's IOPS (e.g., 2000 for good SSD, 200 for HDD)
...
# Threading & Connections
max_connections                = 1000  # Adjust based on application needs. Too high can cause issues.
thread_cache_size              = 200   # Cache for reusable threads
back_log                       = 2000  # Number of pending connections before MySQL stops accepting new ones

# Per-Thread Buffers (Allocated per connection, so keep these reasonable)
sort_buffer_size               = 4M    # For sorting operations
join_buffer_size               = 4M    # For join operations that can't use indexes
read_buffer_size               = 2M    # For sequential scans
read_rnd_buffer_size           = 2M    # For random reads (e.g., after sorting)

# Temporary Tables
tmp_table_size                 = 512M  # Max size for in-memory temporary tables
max_heap_table_size            = 512M  # Max size for user-created MEMORY tables
                                       # Ensure tmp_table_size <= max_heap_table_size
